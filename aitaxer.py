...
st.write('Например, налог в 10% взымается с лиц имеющих доход 0 - 9,950$ в год. ')
st.write('Немного изменим границы для наших ботов, чтобы было проще считать.')
#########Запуск симуляции#######
number1 = 50
col18,col19,col110,col111 = st.columns(4)
with col18:
    ef5 = st.slider('Эффективность 1 бота',0,523601,25000)
with col19:
    ef6 = st.slider('Эффективность 2 бота',0,523601,10000)
with col110:
    ef7 = st.slider('Эффективность 3 бота',0,523601,5000)
with col111:
    ef8 = st.slider('Эффективность 4 бота',0,523601,1000)
but_start2 = st.button('Запустить симуляцию Federal US')
    ...
    for i in range(number1):
        with placeholder2.container():
            ...
            with col14:
                ...
                data_curr = pd.DataFrame({'iter':i,'1 Бот':bot1[1],'2 Бот':bot2[1],'Бот 3':bot3[1],'Бот 4':bot4[1]},index=[0])
                data_line = pd.concat([data_line,data_curr])
                data_line.to_csv('saved_data.csv',encoding='cp1251')
                read_datas = pd.read_csv('saved_data.csv',encoding='cp1251')
                st.line_chart(read_datas)
        bot1[1] += federalus.federaltax(bot1[0])/1000
        bot2[1] += federalus.federaltax(bot2[0])/1000
        bot3[1] += federalus.federaltax(bot3[0])/1000
        bot4[1] += federalus.federaltax(bot4[0])/1000
st.write('И снова убеждаемся в том, что наиболее эффективный бот оказывается в максимальном выигрыше. И снова есть опасность возникновения монополии.')
